...
function isPalindrome(str) {
  ...
  const substrLen = Math.floor(strLen / 2);
  const ClearStr = str.replaceAll(/[^\w]/g, '');
  if (strLen < 2) return true;
  return (
    ClearStr.toLowerCase().slice(0, substrLen) ===
    ClearStr.toLowerCase().split('').reverse().join('').slice(0, substrLen)
  );
}
